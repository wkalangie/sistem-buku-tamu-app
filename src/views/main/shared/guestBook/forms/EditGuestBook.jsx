...
import { getGuestBook, updateGuestBook } from '@/store/slices/guestBook';
...
const EditGuestBook = ({ onClose, data }) => {
    const dispatch = useDispatch();
    const { user } = useAuth();
    const { errorUpdate, loadingUpdate } = useSelector((state) => state.guestBook);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setIsDone(false);
            await dispatch(updateGuestBook(data?.id, values));
            setIsDone(true);
        }
    });
